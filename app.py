...
# Set up basic logging so we can see errors in Render logs
...
def scrape_fed_speeches():
    try:
        ...
        speeches = []
        # Select all links that contain 'speech' in the href
        ...
        return speeches[:3]  # Only latest 3
...
def extract_text_from_speech(url):
    try:
        ...
        if not article:
            return ""
        text = article.get_text()[:4000]
        logging.info(f"Extracted {len(text)} chars from {url[:50]}...")
        return text
    except Exception as e:
